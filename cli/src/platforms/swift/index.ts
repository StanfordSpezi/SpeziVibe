...
export class SwiftPlatformGenerator implements PlatformGenerator {
  ...
  async generate(options: GenerationOptions): Promise<GenerationResult> {
    ...
    try {
      ...
      // 3.6. Handle HealthKit type selection
      if (allFeatures.includes('healthkit') && options.healthKitTypes && options.healthKitTypes.length > 0) {
        spinner = spin('Configuring HealthKit data types...');

        // Build CollectSample lines
        const collectSamples = options.healthKitTypes
          .map((t) => `            CollectSample(.${t})`)
          .join('\n');

        // Update the delegateHelpers in the HealthKit manifest to use selected types
        const healthKitManifest = manifests.get('healthkit');
        if (healthKitManifest) {
          healthKitManifest.delegateHelpers =
            `private var healthKit: HealthKit {\n` +
            `        HealthKit {\n` +
            options.healthKitTypes.map((t) => `            CollectSample(.${t})`).join('\n') + '\n' +
            `        }\n` +
            `    }`;
        }

        spinner.succeed('Configured HealthKit data types');
      }

      // 3.7. Handle Scheduler task configuration
      if (allFeatures.includes('scheduler') && options.schedulerTasks && options.schedulerTasks.length > 0) {
        spinner = spin('Configuring scheduler tasks...');

        const taskDefs = options.schedulerTasks.map((task, i) => {
          const taskId = task.name.toLowerCase().replace(/\s+/g, '-');
          ...
          return (
            ...
            `                id: "${taskId}",\n` +
            `                title: "${task.name}",\n` +
            `                instructions: "Please complete your ${task.name.toLowerCase()}.",\n` +
        ...
        // Write the customized AppScheduler file
        const schedulerPath = path.join(tempDir, 'Sources', 'Schedule', 'AppScheduler.swift');
        ...
        spinner.succeed('Configured scheduler tasks');
      }

      // 3.8. Handle Contacts builder
      if (allFeatures.includes('contacts') && options.contacts && options.contacts.length > 0) {
        spinner = spin('Building contacts list...');

        const contactDefs = options.contacts.map((c) => {
          ...
          if (c.phone) {
            contactOptions.push(`                .call("${c.phone}")`);
          }
          if (c.email) {
            contactOptions.push(`                .email(addresses: ["${c.email}"], subject: "{{DisplayName}} — Inquiry")`);
          }

          let contactStr =
            `        Contact(\n` +
            `            name: PersonNameComponents(givenName: "${givenName}", familyName: "${familyName}"),\n`;
          if (c.title) {
            contactStr += `            title: "${c.title}",\n`;
          }
          if (c.org) {
            contactStr += `            description: "${c.title || 'Provider'}",\n` +
              `            organization: "${c.org}",\n`;
          } else {
            contactStr += `            description: "${c.title || 'Contact'}",\n`;
          }
        ...
        const contactsPath = path.join(tempDir, 'Sources', 'Contacts', 'ContactsView.swift');
        if (await fs.pathExists(contactsPath)) {
          await fs.writeFile(contactsPath, contactsViewContent);
        }

        spinner.succeed('Built contacts list');
      }

      // 3.9. Handle App Icon
      if (options.appIconPath) {
        spinner = spin('Processing app icon...');
        const iconSrcPath = path.resolve(options.appIconPath);
        if (await fs.pathExists(iconSrcPath)) {
          const iconSetDir = path.join(tempDir, 'Sources', 'Assets.xcassets', 'AppIcon.appiconset');
          await fs.ensureDir(iconSetDir);
          const iconDest = path.join(iconSetDir, 'AppIcon.png');
          await fs.copy(iconSrcPath, iconDest);

          // Resize to 1024x1024 and strip alpha using sips (macOS)
          try {
            execSync(`sips -z 1024 1024 "${iconDest}"`, { stdio: 'pipe' });
            // Strip alpha channel
            execSync(`sips -s format png --setProperty formatOptions 100 "${iconDest}"`, { stdio: 'pipe' });
          } catch {
            // sips may not be available on all systems, just copy as-is
          }
          ...
          spinner.succeed('Processed app icon');
        } else {
          spinner.warn(`App icon not found at ${iconSrcPath} — skipping`);
        }
      }
      ...
      await this.applyVariableSubstitution(tempDir, swiftName, options.displayName, options);
...
`;
  ...
  private hexToRgbFloats(hex: string): { r: string; g: string; b: string } {
  ...
  private async applyVariableSubstitution(
    ...
    // Color substitution values
    const primaryColor = options?.primaryColor || '#0077B6';
    const rgb = this.hexToRgbFloats(primaryColor);
    ...
    for (const filePath of files) {
      ...
      try {
        ...
        content = content.replace(/\{\{primaryColorG\}\}/g, rgb.g);
        content = content.replace(/\{\{primaryColorB\}\}/g, rgb.b);
